...
Fig.MainStrings     = {'DataPixx tools installed?','DataPixx box connected?','TDT connected via DB25?','Using EyeLink interface?','Enable video','Enable audio'};
Fig.MainResults     = {Params.DPx.Installed, Params.DPx.Connected, Params.DPx.TDTonDOUT, Params.DPx.UseInterface, Params.DPx.UseVideo, Params.DPx.UseAudio};
...
for n = 1:numel(Fig.MainStrings)
    if n < 5
        ...
        Xpos = [10+(n-5)*130]*Fig.DisplayScale;
        ...
        if Fig.MainResults{n} == 1
            set(Fig.Mh(n),'backgroundcolor',[0 1 0]);
        end
    end
end
...
set(Fig.Mh(6), 'callback', {@ToggleTDTconnection, 4});
...
if Params.DPx.UseInterface == 1
	set(Fig.h(1,1:6), 'enable','off','TooltipString','Analog inputs 0-5 in use for eye signals'); 
    set(Fig.h(1,11:16), 'enable','off');
    set(Fig.h(3,5:24), 'enable','off');
    set(Fig.h(4,5:24), 'enable','off');
end
...
%         end
    ...
    function ToggleTDTconnection(Obj, Event, Indx)
        switch Indx
            ...
            case 2
                Params.DPx.UseInterface = get(Obj, 'value');
                set(Fig.Mdh(4),'backgroundcolor', Fig.DetectionColors(Params.DPx.UseInterface+1,:));
                if Params.DPx.UseInterface == 1
                    Params.DPx.AnalogInAssign(1:6) = 1:6;
                    set(Fig.ChH(1,1:6), 'BackgroundColor', Fig.UsedChanCol);
                    for n = 1:6
                        set(Fig.h(1,n), 'value', Params.DPx.AnalogInAssign(n));
                    end
                    set(Fig.h(1,1:6), 'enable','off','TooltipString','Analog inputs 0-5 in use for eye signals'); 
                  	set(Fig.h(1,11:16), 'enable','off');
                    set(Fig.h(3,5:24), 'enable','off');
                    ...
                    set(Fig.h(1,1:6), 'enable','on','TooltipString','');
                	set(Fig.h(1,11:16), 'enable','on');
                    set(Fig.h(3,5:24), 'enable','on');
                    set(Fig.h(4,5:24), 'enable','on');
                end
            ...
            case 4
                Params.DPx.UseAudio = get(Obj, 'value');
              	if Params.DPx.UseAudio == 1
                    set(Fig.Mh(6),'backgroundcolor',[0 1 0]);
                else
                    set(Fig.Mh(6),'backgroundcolor',Fig.Background);
                end
        end
    end
    ...
    %====================== Add fields for PLDAPS
    function AddPLDAPSfields
        
        Params.DPx.pldaps.enablePropixxCeilingMount     = 0;
        Params.DPx.pldaps.enablePropixxRearProjection   = 0;
        Params.DPx.pldaps.rb3d                          = 0;
        Params.DPx.pldaps.LogOnsetTimestampLevel        = 2;
        Params.DPx.pldaps.use                           = 1;
        Params.DPx.pldaps.useAsEyepos                   = 1;
        Params.DPx.pldaps.useForReward                  = 1;
        Params.DPx.pldaps.adc.bufferAddress             = [];
        Params.DPx.pldaps.adc.channelGains              = 1;
        Params.DPx.pldaps.adc.channelMapping            = 'datapixx.adc.data';
        Params.DPx.pldaps.adc.channelModes              = 0;
        Params.DPx.pldaps.adc.channelOffsets            = 0;
        Params.DPx.pldaps.adc.channels                  = [];
        Params.DPx.pldaps.adc.maxSamples                = 0;
        Params.DPx.pldaps.adc.numBufferFrames           = 600000;
        Params.DPx.pldaps.adc.srate                     = Params.DPx.AnalogInRate;
        Params.DPx.pldaps.adc.startDelay                = 0;
        Params.DPx.pldaps.adc.XEyeposChannel            = [];
        Params.DPx.pldaps.adc.YEyeposChannel            = [];
     	Params.DPx.pldaps.GetPreciseTime.maxDuration	= [];
        Params.DPx.pldaps.GetPreciseTime.optMinwinThreshold = [];
        Params.DPx.pldaps.GetPreciseTime.syncmode       = [];
    end
