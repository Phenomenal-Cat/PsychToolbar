...
StimOnsetSamples      = find(diff(DiodeData) > 0.25)+1;                                    	% Find photodiode onset samples
StimOffsetSamples     = find(diff(DiodeData) < -0.5);                                         % Find photodiode offset samples
...
elseif numel(StimOffsetSamples) > numel(StimOnsetSamples)
    StimOnsetSamples=[1,StimOnsetSamples];
end
