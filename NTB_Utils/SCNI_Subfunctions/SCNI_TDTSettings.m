...
function ParamsOut = SCNI_TDTSettings(Params)
...
if Success < 1                              % If the parameters could not be loaded...
    ...
    Params.TDT.RZ2_IP               = '';
    Params.TDT.NetBIOSname          = 'TDT_UDP_28_3047';
...
%% ========================= UICALLBACK FUNCTIONS =========================
    ...
    %=============== OPEN UDP SOCKET
    function Params = OpenUDP(Params)
        
        port = 
        Params.TDT.UDPsock = pnet('udpsocket', port); 
        if Params.TDT.UDPsock == -1
            error('pnet failed to open a UDP socket!');
        end
        pnet(Params.TDT.UDPsock,'udpconnect','hostname',port);
        
        pnet(Params.TDT.UDPsock,'printf','Hello world!\n');
        pnet(Params.TDT.UDPsock,'close');

        pnet(Params.TDT.UDPsock,'writepacket');
    end
